...
<?php
    $post_id="18"; //you have to give the post id... I have choosen just a randome id.
	  $usr_id="3";  //you have to give the User id..
    $con= mysqli_connect("localhost", "root", "", "db");
    $query="SELECT * FROM comment WHERE id_post='$post_id' ORDER BY id_post desc";
    $sql=mysqli_query($con,$query);

    if (mysqli_num_rows($sql)>0) {
	    while ($row = mysqli_fetch_assoc($sql)) {
        $commentID= $row['id_comment'];
	    	$post_id1 = $row['id_post'];
	    	$usr_id1 = $row['id_user'];//****can featch usr info***
	    	$commentdata = $row['content'];
        $timeOfCmt = $row['date_posted'];
